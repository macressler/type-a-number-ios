...
-(void)finishedWithAuth:(GTMOAuth2Authentication *)auth error:(NSError *)error
{
  ...
  if (error.code == 0 && auth) {
    ...
    NSLog(@"Failed to log into GooglePlus\n\tError=%@\n\tAuthObj=%@",error,auth);
  }
}
...
-(void)refreshInterface
{
  ...
  NSArray *buttonsToManage = [[NSArray alloc] initWithObjects:
                              self.achButton, self.leadsButton, self.easyButton, self.hardButton,
                              self.signOutButton, self.peopleListButton,                              nil];
